...
class Autrics_Unyson_Google_Fonts {
	...
	public static function generate_url() {
		...
		foreach ( self::$data[ 'family' ] as $family => $family_data ) {
			if ( !empty( $family ) ) {
				$family_data[ 'variants' ][900] = $family_data[ 'variants' ][700] = $family_data[ 'variants' ][100] = 1;
...
add_action( 'wp_enqueue_scripts', array( 'Autrics_Unyson_Google_Fonts', 'output_url' ), 9999 );
